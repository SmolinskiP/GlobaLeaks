...
export class AppModule {
  ...
  constructor(private cryptoService:CryptoService, private authenticationService: AuthenticationService,private httpService: HttpService, private idle: Idle) {
    this.initIdleState();
  }
  ...
  onKeepalive() {
    if (this.authenticationService && this.authenticationService.session) {
      this.httpService.requestGetUserSession().subscribe(
        response => {
          this.cryptoService.proofOfWork(response.id).subscribe(
            (ans) => {
              this.authenticationService.chained_session = new TokenResponse(response.id, ans)
            }
          );
        }
      );
